...
import { ArrowLeft, Calendar, University, CreditCard, Folder, Plus, Trash2 } from "lucide-react";
...
export default function PurchasesPage() {
  ...
  const deletePurchaseMutation = useMutation({
    ...
    onError: () => {
      toast({
        title: "Erro",
        description: "Erro ao remover compra. Tente novamente.",
        variant: "destructive",
      });
    },
  });

  const handleDeletePurchase = (purchaseId: string, purchaseName: string) => {
    if (confirm(`Tem certeza que deseja remover a compra "${purchaseName}"?`)) {
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-xl font-medium">Compras Registradas</h1>
            </div>
            <NavigationMenu />
          </div>
        </div>
              ...
              <div className="text-center py-8 text-gray-500">
                {allPurchases.length === 0 ? (
                  <>
                    ...
                    <p className="text-sm">Adicione sua primeira compra para começar o controle.</p>
                  </>
                ) : (
